...
//операторы 
Если выражение условия оператора if истинно (имеет значение true), то выполняется блок кода внутри оператора if.
а, если выражение оказывается ложным, выполняется первый набор кода после окончания оператора if (после закрывающей фигурной скобки).
...
if (age > 18) {
    if (tasks > 500) {
        System.out.println("Добро пожаловать!");//"Добро пожаловать!" тогда, когда age больше 18 лет, и tasks больше 500.
    }
}
if (age > 18 && tasks > 500) {
...
int x = 15;
System.out.println((x % 2 == 0) ? "Число чётное" : "Число нечётное"); //? проверяет на истиность (замена if)

! логическое НЕ //меняет истенное на ложное и убирает не строгость неравенства
|| логическое ИЛИ
&& логическое И
<	меньше //пример условий
>	больше
==	равно
!=	не равно
<=	меньше или равно
